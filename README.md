...
- При создании товара из админ-панели артикул записывается в `barcode` и `sku` варианта Shopify (для сканирования в Shopify POS).
...
- Для Shopify POS сканирование выполняется по `barcode` варианта товара; бот заполняет его из строки `Artikal: XXXXXXXX`.
