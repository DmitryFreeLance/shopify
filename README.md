...
- Для Shopify POS сканирование выполняется по `barcode` варианта товара; бот формирует barcode из строки `Artikal: XXXXXXXX` как `27` + `XXXXXXXX`, чтобы избежать проблем с лидирующими нулями при распознавании.
