...
- При создании товара из админ-панели `sku` сохраняется как артикул из 8 цифр, а `barcode` записывается в POS-совместимом 10-значном виде без лидирующих нулей в начале кода.
...
- Для Shopify POS сканирование выполняется по `barcode` варианта товара; бот формирует barcode из строки `Artikal: XXXXXXXX` как `99` + `XXXXXXXX`, чтобы избежать проблем с лидирующими нулями при распознавании.
