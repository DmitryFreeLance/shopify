...
- При создании товара из админ-панели `sku` сохраняется как артикул из 8 цифр, а `barcode` записывается в POS-совместимом виде для бирки Code 128.
...
- Для Shopify POS сканирование выполняется по `barcode` варианта товара; бот формирует barcode из строки `Artikal: XXXXXXXX` в том формате, который считывается с бирки.
